...
class TokenizedDataset(Dataset):
    def __init__(self, data_dir, block_size):
        ...
        # Важно: dtype должен быть тот же, что и при сохранении! В нашем скрипте это np.int32
        dtype = np.int32

        # 3. Открываем файл с помощью memmap в режиме только для чтения ('r')
    ...
    def __getitem__(self, idx):
        ...
        full_seq = torch.from_numpy(fragment.astype(np.int64))
...
def train_model_with_saving():
    ...
    for epoch in range(100):  # Максимум 100 эпох
        ...
        if patience >= max_patience:
            print("⏹️ Обучение остановлено из-за отсутствия улучшений.")
            break
